...
class ShchakimIntegration {
  ...
  async updatePrayerTimes(prayers) {
    ...
    weekdayPrayers.forEach((prayer, index) => {
      let element = null;
      
      if (prayerElements[index]) {
        element = prayerElements[index];
        console.log(`[PRAYER] Using weekday element at index ${index}`);
      } else {
        console.warn(`[PRAYER] No element found at index ${index} for weekday prayer`);
      }
