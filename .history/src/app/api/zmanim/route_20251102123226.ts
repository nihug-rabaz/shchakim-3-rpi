...
export async function POST(req: Request) {
  ...
  // Format Hebrew year as תש"ה or תשפ"ה
  let hebrewYearFormatted;
  if (hebrewYearStr.length === 4) {
    const lastDigit = hebrewYearStr.slice(-1);
    const secondLastDigit = hebrewYearStr.slice(-2, -1);
    if (secondLastDigit === '0') {
      hebrewYearFormatted = `תש${lastDigit}"ה`;
    } else {
      hebrewYearFormatted = `תש${secondLastDigit}${lastDigit}"ה`;
    }
  } else {
  ...
  const hebrewDateFormatted = `${hebrewDay} ${hebrewMonth} ${hebrewYearFormatted}`;
