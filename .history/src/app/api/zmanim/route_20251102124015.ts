...
export async function POST(req: Request) {
  ...
  try {
    ...
    const hebrewDateFromAPI = zmanimData.hebrewDate || zmanimData.hebrew?.date;
    const parasha = zmanimData.parasha || zmanimData.parashaName;
    const times = zmanimData.times || zmanimData.zmanim || {};
  ...
  let parasha = null;
  try {
    // Check if today is Shabbat (day 6 = Friday evening, but we check Saturday)
    const dayOfWeek = gregorianDate.getDay();
    
    // Get parasha for the upcoming Shabbat (or current if it's Shabbat)
    let checkDate = new Date(gregorianDate);
    
    // If it's Shabbat, use today. Otherwise find the next Shabbat
    if (dayOfWeek !== 6) {
      const daysUntilShabbat = (6 - dayOfWeek + 7) % 7;
      checkDate = new Date(checkDate.getTime() + daysUntilShabbat * 24 * 60 * 60 * 1000);
    }
    
    const shabbatHDate = new HDate(checkDate);
    const parashaObj = Parasha.getParsha(shabbatHDate, true); // true = Israel
    if (parashaObj) {
      parasha = parashaObj.getName('h'); // Hebrew name
    }
  } catch (error) {
    // If Parasha.getParsha doesn't work, try alternative method
    try {
      // Alternative: use hebcal's calendar events
      const { HebrewCalendar, Event } = await import('@hebcal/core');
      const events = HebrewCalendar.getHolidaysOnDate(hebrewDate, true);
      const parashaEvent = events.find(e => e instanceof Parasha);
      if (parashaEvent) {
        parasha = parashaEvent.getName('h');
      }
    } catch (altError) {
      console.error('Error getting parasha:', altError);
    }
  }
