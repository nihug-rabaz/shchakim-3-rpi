...
export async function POST(req: Request) {
  ...
  try {
    ...
    let parasha = zmanimData.parasha || zmanimData.parashaName || zmanimData.parsha || zmanimData.torahPortion;
  ...
  const hebrewDateFormatted = `${hebrewDayStr} ${hebrewMonthStr} ${hebrewYearStr}`;
    ...
    // If parasha missing from API, compute upcoming Shabbat parasha locally (Israel minhag)
    if (!parasha) {
      try {
        const dow = gregorianDate.getDay();
        const daysUntilShabbat = (6 - dow + 7) % 7;
        const shabbatDate = new Date(gregorianDate);
        shabbatDate.setDate(gregorianDate.getDate() + daysUntilShabbat);
        const shabbatHDate = new HDate(shabbatDate);
        const p = Parasha.getParsha(shabbatHDate, true);
        parasha = Array.isArray(p) ? p.map((x: any) => x.getName('h')).join('-') : p?.getName?.('h') || null;
      } catch {}
    }
