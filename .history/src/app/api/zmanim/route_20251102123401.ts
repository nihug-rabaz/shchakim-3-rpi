...
export async function POST(req: Request) {
  ...
  function formatHebrewYear(year: number): string {
    // Hebrew year conversion table for common years (5780-5799)
    ...
    // Fallback: convert year to Hebrew letters
    ...
    if (isNaN(yearNum)) {
      return `תש"ה`; // Default fallback
    }
    
    if (yearNum < 10) {
      return `תש"${ones[yearNum] || 'ה'}`;
    } else {
      const tensDigit = Math.floor(yearNum / 10);
      const onesDigit = yearNum % 10;
      
      // Check if tens and ones digits are valid
      if (tensDigit >= tens.length || onesDigit >= ones.length) {
        return `תש"ה`; // Default fallback
      }
      
      const tensLetter = tens[tensDigit] || '';
      const onesLetter = ones[onesDigit] || 'ה';
      
      if (tensDigit === 8) {
        return `תש${tensLetter}"${onesLetter}`;
      } else if (tensDigit === 0) {
        return `תש"${onesLetter}`;
      } else {
        // For other cases (70s, 90s, etc.)
        return `תש${tensLetter}${onesLetter}"ה`;
      }
    }
  }
