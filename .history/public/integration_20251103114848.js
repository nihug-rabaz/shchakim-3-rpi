...
class ShchakimIntegration {
  ...
  ensurePolinFont() {
    ...
    style.textContent = `
@font-face { font-family: 'Polin'; src: url('/fonts/Polin-Regular.woff2') format('woff2'), url('/fonts/Polin-Regular.woff') format('woff'), url('/fonts/Polin-Regular.ttf') format('truetype'); font-weight: 400; font-style: normal; font-display: swap; }
@font-face { font-family: 'Polin'; src: url('/fonts/Polin-Bold.woff2') format('woff2'), url('/fonts/Polin-Bold.woff') format('woff'), url('/fonts/Polin-Bold.ttf') format('truetype'); font-weight: 700; font-style: normal; font-display: swap; }
      ...
      const injectHalacha = async () => {
        try {
          ...
          const mkHalachaSlide = (title, summary) => {
            ...
            card.style.padding = '24px 28px 18px';
          ...
          if (items[0] && slides[1]) {
            ...
            if (footer) {
              const chapter = items[0].chapter ? `פרק ${items[0].chapter}` : '';
              const credit = items[0].credit ? ` – מתוך ${items[0].credit}` : ' – מתוך קיצור תורת המחנה של הרבנות הצבאית';
              footer.textContent = `${chapter}${credit}`.trim();
            }
          ...
          if (items[1] && slides[2]) {
            ...
            if (footer2) {
              const chapter2 = items[1].chapter ? `פרק ${items[1].chapter}` : '';
              const credit2 = items[1].credit ? ` – מתוך ${items[1].credit}` : ' – מתוך קיצור תורת המחנה של הרבנות הצבאית';
              footer2.textContent = `${chapter2}${credit2}`.trim();
            }
