    ...
    try {
      ...
      const injectHalacha = async () => {
        try {
          ...
          if (items[0] && slides[1]) {
            slides[1].innerHTML = '';
            slides[1].appendChild(mkHalachaSlide(items[0].title, items[0].summary));
            slides[1].style.opacity = '0';
          }
          if (items[1] && slides[2]) {
            slides[2].innerHTML = '';
            slides[2].appendChild(mkHalachaSlide(items[1].title, items[1].summary));
      ...
      const intervalMs = 30000; // 30 seconds to allow more time for reading
      setInterval(() => {
        const prev = current;
