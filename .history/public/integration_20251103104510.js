...
class ShchakimIntegration {
  ...
  setupSlider() {
    try {
      ...
      // Hide the original image mentioned by selector if present
      ...
      target.style.borderRadius = target.style.borderRadius || '30px';

      // Determine size from computed styles
      const rect = target.getBoundingClientRect();
      const width = rect.width || target.clientWidth || 800;
      const height = rect.height || target.clientHeight || 450;
      ...
      // Slide 1: use the provided image src if available
      const imgSrc = originalImage?.getAttribute('src') || originalImage?.getAttribute('data-savepage-src');
      if (imgSrc) {
        makeSlide(imgSrc);
      } else {
        makeSlide({ type: 'color', value: this.hexToRgba(this.themeColor, 0.25) });
      }

      // Slides 2-4: placeholders themed; replace later if content arrives
      makeSlide({ type: 'color', value: this.hexToRgba(this.themeColor, 0.35) });
      makeSlide({ type: 'color', value: this.hexToRgba(this.themeColor, 0.45) });
      makeSlide({ type: 'color', value: this.hexToRgba(this.themeColor, 0.55) });

      // If content API includes images, replace backgrounds
      try {
        const images = this.content?.promos?.images || this.content?.images || [];
        images.slice(0, 4).forEach((img, idx) => {
          const url = typeof img === 'string' ? img : img?.url;
          if (url && slides[idx]) slides[idx].style.backgroundImage = `url(${url})`;
        });
      } catch {}
