...
class ShchakimIntegration {
  ...
  async updateDate(element) {
    try {
      ...
      // Call zmanim API to get Hebrew date
  ...
  formatHebrewDate(zmanimData, date) {
    // The zmanim API should return Hebrew date in the response
    // If not, we'll construct it from the date
    if (zmanimData.hebrewDate) {
      return zmanimData.hebrewDate;
    }
    
    // Fallback: basic Hebrew date formatting
    // You might need to adjust this based on the actual API response structure
    const day = date.getDate();
    const month = date.getMonth() + 1;
    const year = date.getFullYear();
    
    // Hebrew month names (approximate mapping)
    const hebrewMonths = [
      'תשרי', 'חשון', 'כסלו', 'טבת', 'שבט', 'אדר',
      'ניסן', 'אייר', 'סיון', 'תמוז', 'אב', 'אלול'
    ];
    
    // For now, return a placeholder that will be updated with actual Hebrew date from API
    // The API should provide the Hebrew date in its response
    return `${day} ${hebrewMonths[month - 1]} תשפ"ה`;
  }
