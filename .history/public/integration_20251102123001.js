...
class ShchakimIntegration {
  ...
  formatHebrewDate(zmanimData, date) {
    // The zmanim API might return Hebrew date in various formats
    ...
    // If API has day/month/year fields
    if (zmanimData.hebrew?.day && zmanimData.hebrew?.month && zmanimData.hebrew?.year) {
      ...
      return `${day} ${hebrewMonths[monthIndex]} תשפ"ה`;
    }
    
    // Fallback: try to parse from date field or use current date approximation
